...
export default function GlobalChatbot() {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 font-sans flex items-end ${
        isOpen ? "flex-col-reverse sm:flex-row gap-4" : "flex-col"
      }`}
      id="global-chatbot-fixed-root"
    >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="relative p-4 bg-fresh-green hover:bg-[#2E7D32] text-white rounded-full shadow-2xl transition-all flex items-center justify-center cursor-pointer border-2 border-emerald-100 group flex-shrink-0"
      ...
      <AnimatePresence>
        ...
        {isOpen && (
          <motion.div
            ...
            className="w-[92vw] sm:w-[500px] max-w-[520px] h-[680px] max-h-[calc(100vh-120px)] min-h-[400px] bg-[#EBF3EE] border border-[#C2DACB] shadow-2xl rounded-3xl overflow-hidden flex flex-col justify-between transform origin-bottom-right"
          >
            {/* Header branding */}
            <div className="bg-[#D0E2D6] px-5 py-4 border-b border-[#B8D1C1] flex items-center justify-between">
              <div className="flex items-center gap-3 text-emerald-950">
                <div className="p-2 bg-white/70 rounded-xl border border-[#A6C5B2] shadow-2xs">
                  <Bot className="h-5.5 w-5.5 text-[#2E7D32]" />
                </div>
                <div className="text-left">
                  <h3 className="font-display font-black text-sm tracking-tight text-emerald-950">
                    CHSP Care Assistant
                  </h3>
                  <p className="text-[10px] text-[#1E4D2B] font-extrabold flex items-center gap-1.5">
              ...
              <button
                onClick={handleToggleOpen}
                className="p-1 px-1.5 bg-[#C2DACB] hover:bg-[#B3CEBD] text-[#1E4D2B] hover:text-[#0C2412] rounded-lg transition-colors cursor-pointer"
            ...
            {/* Advisory alert bar */}
            <div className="p-3 bg-[#F1F7F3] text-left border-b border-[#C2DACB]/60">
              <p className="text-[11px] text-stone-700 leading-normal flex items-start gap-1.5 font-bold">
                <Info className="h-4.5 w-4.5 text-[#2E7D32] flex-shrink-0 mt-0.5" />
                <span>
            ...
            {/* Chats history panel */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#EBF3EE]">
              {messages.map((m, idx) => (
                <div
                  key={idx}
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-3 text-xs sm:text-sm leading-relaxed ${
                      m.role === "user"
                        ? "bg-[#2E7D32] text-white rounded-tr-none font-semibold shadow-xs"
                        : "bg-white text-stone-850 border border-[#CBDCD1] rounded-tl-none font-medium shadow-2xs pr-3 text-left"
                    }`}
                  >
                    {m.role === "model" ? (
            ...
            <div className="p-3 bg-[#E3EFE6] border-t border-[#C2DACB] text-left">
              <span className="text-[10px] uppercase font-bold text-emerald-800 flex items-center gap-1.5 mb-1.5">
                <HelpCircle className="h-3 w-3 text-[#2E7D32]" />
              ...
              <div className="flex flex-col gap-1 max-h-[110px] overflow-y-auto">
                {quickPrompts.map((p, pIdx) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="text-[11px] text-left font-bold text-stone-800 bg-white hover:bg-[#2E7D32]/10 hover:text-[#2E7D32] border border-[#CCDCD1] rounded-lg p-2.5 transition-all cursor-pointer truncate"
                  >
                    {p.label}
                  </button>
                ))}
            ...
            <form onSubmit={handleMessageSubmit} className="p-3 border-t border-[#C2DACB] bg-[#DCEAE0] flex items-center gap-2">
              <input
                type="text"
                value={inputVal}
                ...
                className="flex-1 rounded-2xl border border-[#B8D1C1]/80 px-4 py-3 text-xs sm:text-sm bg-white outline-none focus:border-[#2E7D32] focus:ring-4 focus:ring-[#2E7D32]/15 transition-all font-semibold text-stone-850"
              />
              <button
                type="submit"
                disabled={isLoading || !inputVal.trim()}
                className="p-3.5 bg-[#2E7D32] hover:bg-[#1B5E20] disabled:bg-[#B3CEBD] text-white rounded-2xl transition-all duration-200 cursor-pointer flex-shrink-0 shadow-sm"
              >
